...
multiDialogue = { "Kellina: Soulusek's eye, do I even have to think for you now? Nevermind. What I require is a plain robe, and a silk cord.",
...
npcDialogue = "Kellina: Do you fancy yourself your own master? You had best reconsider getting this done or we will have to reconsider your enrollment here."
...
elseif (GetPlayerFlags(mySession, "100106") == "0") then
 if (level >=7) then
npcDialogue = "Kellina: playerName, you'll need to learn to speak up for yourself, or you won't get far in this city."
if (choice:find("assignment")) then
...
if (CheckQuestItem(mySession, items.VENOM_SAC, 1))
 then
...
"You have given away a Venom Sac.",
...
TurnInItem(mySession, items.VENOM_SAC, 1)
...
elseif (GetPlayerFlags(mySession, "100110") == "0") then
 if (level >=10) then
npcDialogue = "Kellina: Declining to help your teacher will not look good on your records, I assure you, playerName."
if (choice:find("Kellina")) then
...
CompleteQuest(mySession, 100115, quests[100115][5].xp)
...
multiDialogue = { "Kellina: I have received word from Ilenar. He is very pleased.",
    "Kellina: As payment for your services, you've earned the right to choose a new ability and a weapon. The first is a Frozen Mark Scroll, an ability that enhances your pet. It comes with a Magical Staff.",
    "Kellina: The second choice is a Lava Stone Scroll, which summons a charged item with a potent damage spell. It comes with a Mystical Tome.",
    "Kellina: If you continue to choose the same type of ability from this point forward, you can eventually specialize."
}
SendMultiDialogue(mySession, multiDialogue)
    npcDialogue = "So, which do you choose?"
    diagOptions = { "Frozen Mark Scroll with a Magical Staff.", "Lava Stone Scroll with a Mystical Tome." }
if (choice:find("Mystical")) then
npcDialogue = "You have received a Lava Stone Scroll."
elseif (choice:find("Magical")) then
    multiDialogue = {
...
CompleteQuest(mySession, 100120, quests[100120][9].xp)
