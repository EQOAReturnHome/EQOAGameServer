...
function event_say()
   ...
   then
      ...
      if (ch:find("apprentice")) then
         diagOptions = {}
         npcDialogue = "Malsis: Oooohh, well we do require that a prospective apprentice complete a number of tasks before the enrollment."
         SendDialogue(mySession, npcDialogue, diagOptions)
         npcDialogue = "Malsis: Your first task is to acquire an iron ring from the merchant outside. Her name is Yulia. She wont charge you for the ring."
         SendDialogue(mySession, npcDialogue, diagOptions)
   ...
   elseif (GetPlayerFlags(mySession, "EasternMagician1")) then
      ...
      then
         ...
         npcDialogue = "Malsis: I have no time to offer odd jobs to every transient that decides to waltz into the Academy!!! I'll have you know, We are quite busy."
      end
      SendDialogue(mySession, npcDialogue, diagOptions)
   end
end
