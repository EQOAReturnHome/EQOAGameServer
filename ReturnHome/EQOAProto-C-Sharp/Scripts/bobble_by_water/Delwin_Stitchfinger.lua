...
function event_say()
    ...
    if (GetPlayerFlags(mySession, "10017") == "2") then
        ...
        elseif (ch:find("terrible")) then
            if(CheckQuestItem(mySession, 8457, 1)) then
               SetPlayerFlags(mySession, "10017", "3")
                ...
                questText = "Go see Grocer Fritz, purchase some fine chocolate and return to Delwin Stitchfinger."
        ...
        elseif (ch:find("sorry")) then
           multiDialogue = {"Delwin Stitchfinger: I know what you mean. It is a lavish delacacy, but one that I can't live without.",
           "Delwin Stitchfinger: Welp, I hope you have a pleasant day."}
           SendMultiDialogue(mySession, multiDialogue)
        else
    ...
    elseif (GetPlayerFlags(mySession, "10017") == "3") then
        if (CheckQuestItem(mySession, 8335, 1) then
            ...
            elseif (ch:find("Yes")) then
                ...
                DeleteQuestLog(mySession, 0)
                questText = "Return to Delwin after a little while."
                AddQuestLog(mySession, 0, questText)
                SetPlayerFlags(mySession, "10017", "4")
            else
    ...
    elseif (GetPlayerFlags(mySession, "10017") == "4") then
        ...
        SetPlayerFlags(mySession, "10017") == "5")
    elseif (GetPlayerFlags(mySession, "10017") == "6") then
        if (CheckQuestItem(mySession, 8337, 1) then
            ...
            elseif (ch:find("Yes")) then
                ...
                questText = "Return to Ilenar Crelwin. Tell him of Delwin's fate."
                AddQuestLog(mySession, 0, questText)
                SetPlayerFlags(mySession, "10017", "7")
            else
                npcDialogue = "Well hello there, have you something I can mend?"
